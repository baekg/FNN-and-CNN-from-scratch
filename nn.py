...
class NeuralNetwork:
	...
	def computeLoss(self, Y, predictions):
		...
		# def fun(i):
		# 	if Y[i]:
		# 		return -np.log(predictions[i])
		# 	else:
		# 	return -np.log(1- predictions[i])
		# return np.vectorize(fun)(range(len(Y)))
		temp=-np.log(predictions[-1])*Y-(np.log(1-predictions[-1]))*(1-Y)
		return np.sum(temp)

		# return np.array([-np.log(predictions[-1][i][0]) if Y[i]==1 else -np.log(1- predictions[-1][i][0]) for i in range(len(Y))])
	...
	def backpropagate(self, activations, Y):
		...
		# del_sig=[-Y[i]/activations[-1][i][0]+(1-Y[i])/activations[-1][i][1] for i in range(len(Y))]
		# del_sig=np.zeros(activations[-1].shape)
		inv=-1/activations[-1]
		inv2=1/(1-activations[-1])
		del_sig=inv*Y+inv2*(1-Y)
		# del_sig=del_sig.reshape((len(del_sig),1))
		n=len(activations)
		# print("------layers",len(self.layers))
		# print("-----activations",len(activations))
		# print("del_sig",del_sig)
		# print("activations",activations)

		while n>=2:
			# print(n)
			l=self.layers[n-2]
			del_sig=l.backwardpass(self.alpha,activations[n-1],activations[n-2],del_sig)
			n-=1
		return
